...
const chatSlice = createSlice({
	...
	reducers: {
		...
		addChat: (
			...
			state.threads[action.payload.threadId].chats.push(
				action.payload.chat
			);
		},
		updateMode: (
			...
			if (
				...
				state.threads[threadId].chats[chatIndex]
			) {
				state.threads[threadId].chats[chatIndex].mode = mode;
				state.threads[threadId].chats[chatIndex].arg = arg;
				console.log(
					'inside chatSlice.ts after state update',
					state.threads[threadId].chats
				);
			}
		},
		addMessage: (
		...
		updateSearch: (
			...
			state.threads[threadId].chats[chatIndex].searchResults =
				searchResults;
		},
		updateWeather: (
			state,
			...
			state.threads[threadId].chats[chatIndex].weatherResults =
				weatherResults;
		},
		updateStock: (
			state,
			...
			state.threads[threadId].chats[chatIndex].stocksResults =
				stocksResults;
		},
		updateDictionary: (
			state,
			...
			const { threadId, chatIndex, dictionaryResults } =
				action.payload;
